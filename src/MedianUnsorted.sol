...
contract MedianUnsorted {
    uint256[] sorted;
    uint256[] unsorted;

    function findMedian(uint256[] memory unsorted_) external returns(uint256 median) {
        median = _findMedian(unsorted_);
    }
    ...
    function _findMedian(uint256[] memory unsorted_) internal  returns(uint256 median) {
        uint256 length = unsorted_.length;
        uint256[] memory sorted_ = _sort(unsorted_);
        if(_isEven(length)) {
            uint256 b = length / 2;
            uint256 a = b - 1;
